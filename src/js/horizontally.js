...
function setOptions(obj) {
    ...
    if (
        ...
        typeof options.wrapper != "object"
    ) {
        throw new Error(
            "Incorrect wrapper. Please check for spelling mistakes or take a look at the documentation example."
        );
    }
}
...
function addArrowButtonsToDom() {
    ...
    options.wrapper.insertAdjacentHTML(
        "afterend",
        `<div id="fixed-arrows">` + leftArrowHTML + rightArrowHTML + `</div>`
    );
...
function addPageSelectorsToDom() {
    ...
    state.sections.forEach((element, index) => {
        ...
        } data-index="${index}"><span></span></li>`;
    });

    // Adds the circle page selector html after the wrapper
    options.wrapper.insertAdjacentHTML(
        "afterend",
        `<div id="page-selector"><ul>` + innerHTML + `</ul></div>`
    );
...
function createSectionArray() {
    ...
    if (sections.length > 0) {
        ...
        throw new Error(
            "Could not find correct section class. Please check for spelling mistakes or take a look at the documentation example."
        );
    }
}
...
function updateState() {
    ...
    state.currentIndex = state.sections.findIndex(
        (index) => index === state.currentSection
    );
    ...
    state.nextSection =
        state.currentIndex === state.sections.length - 1
            ? state.sections[0]
            : state.sections[state.currentIndex + 1];
...
function handleTouchMove(e) {
    if (!state.xTouchPos || !state.yTouchPos) {
        return;
    }
